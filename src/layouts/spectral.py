...
def spectral_layout(G: nx.Graph, **kwargs) -> Dict[Any, Tuple[float, float]]:
    """Compute node positions using the eigenvectors of the graph Laplacian.

    Falls back to the largest connected component if the graph is disconnected.
    """
    subgraph = _largest_connected_component(G)
    return nx.spectral_layout(subgraph, **kwargs)
